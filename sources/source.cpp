...
void hashConfig(Json& j) {
  while (continueProcess) {
    ...
    std::string hashEnding = hash.substr(hash.size() - endSequenceLength);

    if (hashEnding == endSequence) {
      BOOST_LOG_TRIVIAL(info) << endSequence << " founded in hash \"" << hash
                              << "\" of string \"" << genstring << "\"";
      if (makeJSON) {
